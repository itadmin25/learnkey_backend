        ...
        for (const countryObj of data.countries) {
            ...
            for (const stateObj of countryObj.states) {
                ...
                const state = await State.findOneAndUpdate(
                    { name: stateObj.name, country: country._id },
                    { name: stateObj.name, country: country._id },
                    { new: true, upsert: true }
                );

                for (const classObj of stateObj.classes) {
                    // Insert class
                    const classDoc = await Class.findOneAndUpdate(
                        { class: classObj.class, state: state._id },
                        { class: classObj.class, state: state._id },
                        { new: true, upsert: true }
                    );

                    for (const yearObj of classObj.years) {
                        // Insert year (reference both state and class)
                        const year = await Year.findOneAndUpdate(
                            { year: yearObj.year, state: state._id, class: classDoc._id },
                            { year: yearObj.year, state: state._id, class: classDoc._id },
                            { new: true, upsert: true }
                        );
